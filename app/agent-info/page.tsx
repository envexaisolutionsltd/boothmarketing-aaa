...
export default function AgentInfoPage() {
  return <main className="mx-auto max-w-4xl px-6 py-16 text-[#f4f4f3]">
    ...
    <section className="mt-12"><h2 className="text-2xl font-semibold">Services</h2><div className="mt-5 space-y-5">{[...company.primaryServices,...company.secondaryServices].map(service=><article key={service.name}><h3 className="font-semibold">{service.name}</h3><p className="mt-1 text-[#8b8b91]">{service.description}</p></article>)}</div></section>
    ...
    <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify({ '@context':'https://schema.org','@type':'ProfessionalService',name:company.companyName,url:company.domain,description:company.whatWeDo,knowsAbout:[...company.primaryServices,...company.secondaryServices].map(s=>s.name) }).replace(/</g,'\\u003c')}} />
  </main>
}
