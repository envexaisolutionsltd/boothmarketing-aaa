...
const approach = [
  ...
  ['You decide what happens next', 'You get a clear view of what is worth changing and what should stay human. There is no pressure to continue into a build.'],
]
...
function SectionHeading({ eyebrow, title, copy }: { eyebrow: string; title: string; copy?: string }) {
  return <div className="max-w-[830px]">
    ...
    <h2 className="mt-[22px] text-[clamp(36px,4vw,52px)] font-semibold leading-[1.06] tracking-[-0.045em] text-[#f4f4f3]">{title}</h2>
    {copy && <p className="mt-7 max-w-[730px] text-[17px] leading-[1.75] text-[#929399]">{copy}</p>}
  </div>
}
...
export default function Page() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#090a0b] text-[#f4f4f3] selection:bg-[#efe3cf] selection:text-[#151515]">
    ...
    <section id="top" className="min-h-[680px] border-b border-[#1a1c1e] bg-[radial-gradient(circle_at_49%_41%,rgba(255,255,255,0.018),transparent_39%)]">
      <div className="mx-auto w-[min(1112px,calc(100%-40px))] pt-16 sm:pt-20 lg:pt-24">
        <FadeIn className="max-w-[820px]">
          ...
          <h1 className="mt-[38px] max-w-[800px] text-[clamp(50px,5.75vw,73px)] font-semibold leading-[0.99] tracking-[-0.062em] text-[#f4f4f3]">Your business has grown. <span className="text-[#ceced1]">Too much of the operation is still being held together manually.</span></h1>
          <p className="mt-[35px] max-w-[705px] text-[19px] leading-[1.55] text-[#b1b1b6]">Booth Marketing helps established businesses identify where repetitive work, disconnected processes and manual handoffs are consuming capacity, then shows you what is actually worth automating.</p>
          <div className="mt-8"><PrimaryLink /></div>
          <p className="mt-4 text-sm text-[#626369]">No technical brief needed. No commitment. Start with one real workflow.</p>
        </FadeIn>
      </div>
    </section>

    <section className="border-b border-[#1a1c1e] py-20 sm:py-24">
      <div className="mx-auto w-[min(1112px,calc(100%-40px))]">
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-2">
          {frictionPoints.map(([title, copy], index) => <FadeIn key={title} delay={index * 0.04} className={index === frictionPoints.length - 1 ? 'md:col-span-2' : ''}><article className="h-full rounded-[15px] border border-[#2b2d30] bg-[#0f1113] p-7 sm:p-8"><small className="text-[10px] text-[#c9bca8]">0{index + 1}</small><h3 className="mt-8 text-[18px] font-semibold leading-[1.32] tracking-[-0.025em] text-[#ededed]">{title}</h3><p className="mt-4 text-sm leading-[1.7] text-[#77787f]">{copy}</p></article></FadeIn>)}
        </div>
        <div className="mt-8 flex min-h-[58px] items-center rounded-xl border-l-2 border-[#d92f3c] bg-[#190f11] px-6 text-[15px] text-[#bdb8ba]">If the business only runs smoothly because good people remember everything, the process is carrying more risk than it should.</div>
      </div>
    </section>

    <section className="border-b border-[#1a1c1e] bg-[#0d0f10] py-20 sm:py-24">
      <div className="mx-auto grid w-[min(1112px,calc(100%-40px))] gap-12 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <FadeIn delay={0.08}><div className="overflow-hidden rounded-[17px] border border-[#2b2d30] bg-[#0b0d0e]">
          ...
          ].map(([title, copy], index) => <div key={title} className="grid grid-cols-[46px_1fr] gap-3 border-b border-[#232528] px-6 py-5 last:border-0"><span className="pt-1 text-[10px] text-[#c7b9a4]">0{index + 1}</span><div><p className="font-medium text-[#e8e8e8]">{title}</p><p className="mt-1 text-sm text-[#626369]">{copy}</p></div></div>)}
          <p className="border-t border-[#232528] px-6 py-5 text-sm leading-7 text-[#999aa0]">Your people should use judgment where it matters. They should not have to be the connection between every system and next step.</p>
        </div></FadeIn>
      </div>
    </section>

    <section className="border-b border-[#1a1c1e] py-24">
      <div className="mx-auto w-[min(1112px,calc(100%-40px))]">
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {approach.map(([title, copy], index) => <FadeIn key={title} delay={index * 0.05}><article className="h-full min-h-[250px] rounded-[15px] border border-[#2b2d30] bg-[#0f1113] p-7"><span className="grid h-10 w-10 place-items-center rounded-lg border border-[#3a3c3f] text-[11px] text-[#d2c5b1]">0{index + 1}</span><h3 className="mt-8 text-[18px] font-semibold tracking-[-0.03em]">{title}</h3><p className="mt-4 text-sm leading-[1.72] text-[#77787e]">{copy}</p></article></FadeIn>)}
        </div>
      </div>
    </section>

    <section className="border-b border-[#1a1c1e] bg-[#0d0f10] py-24">
      <div className="mx-auto w-[min(1112px,calc(100%-40px))]">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
          <FadeIn><div className="rounded-[17px] border border-[#2b2d30] bg-[#0b0d0e] p-7 sm:p-8"><div className="space-y-7"><CalculatorInput label="Team members doing repetitive admin" value={teamMembers} min={1} max={50} suffix="people" onChange={setTeamMembers} /><CalculatorInput label="Average loaded hourly staff cost" value={hourlyCost} min={10} max={100} prefix="£" suffix="/ hr" onChange={setHourlyCost} /><CalculatorInput label="Hours per person spent on repetitive work each week" value={hoursPerWeek} min={1} max={30} suffix="hrs" onChange={setHoursPerWeek} /><CalculatorInput label="Working weeks per year" value={weeksPerYear} min={40} max={52} suffix="weeks" onChange={setWeeksPerYear} /><CalculatorInput label="Reduction scenario" value={reduction} min={10} max={80} suffix="%" onChange={setReduction} /></div></div></FadeIn>
          <FadeIn delay={0.08}><div className="rounded-[17px] border border-[#2b2d30] bg-[#0f1113] p-7 sm:p-8"><p className="text-[10.5px] font-extrabold uppercase tracking-[0.28em] text-[#d8cbb7]">Based on your inputs</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><ResultCard icon={Clock3} label="Repetitive work" value={`${number(calculator.weeklyHours)} hrs / week`} /><ResultCard icon={Gauge} label="Estimated monthly staff cost" value={currency(calculator.monthlyCost)} /><ResultCard icon={BarChart3} label="Estimated annual staff cost" value={currency(calculator.annualCost)} accent /><ResultCard icon={Users} label="Annual hours consumed" value={`${number(calculator.annualHours)} hrs`} /></div>
    ...
    <section className="border-b border-[#1a1c1e] bg-[#0d0f10] py-20 sm:py-24">
      <div className="mx-auto w-[min(1112px,calc(100%-40px))]">
        <FadeIn className="rounded-[17px] border border-[#2b2d30] bg-[#0b0d0e] p-8 sm:p-10 lg:p-12"><div className="grid gap-10 lg:grid-cols-[1fr_0.8fr] lg:items-center"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.28em] text-[#d8cbb7]">Free Automation Audit</p><h2 className="mt-5 text-[clamp(36px,4vw,52px)] font-semibold leading-[1.06] tracking-[-0.045em]">Leave knowing what is worth automating, what is not, and where to start.</h2><p className="mt-6 max-w-2xl text-base leading-7 text-[#929399]">In a focused session, we review how a real process currently works, identify where unnecessary handling is happening, and give you a clear picture of what could realistically change.</p></div><div className="space-y-3">{['No cost and no commitment', 'Specific to your workflows, not generic advice', 'Honest assessment, including what is not worth automating'].map(item => <div key={item} className="flex items-start gap-3 rounded-xl border border-[#2b2d30] bg-[#0f1113] px-4 py-4 text-sm text-[#bdbdc1]"><Check className="mt-0.5 h-4 w-4 shrink-0 text-[#d8cbb7]" />{item}</div>)}</div></div></FadeIn>
      </div>
    </section>
    ...
    <section className="border-b border-[#1a1c1e] py-24">
      <div className="mx-auto w-[min(1112px,calc(100%-40px))]">
        ...
        <div className="mt-12 border-t border-[#232528]">{nextSteps.map(([title, copy], index) => <FadeIn key={title} delay={index * 0.04}><div className="grid gap-4 border-b border-[#232528] py-7 md:grid-cols-[62px_minmax(220px,0.8fr)_1.2fr]"><small className="pt-1 text-[10px] text-[#c7b9a4]">0{index + 1}</small><h3 className="text-[18px] font-semibold text-[#e8e8e8]">{title}</h3><p className="max-w-[620px] text-sm leading-[1.75] text-[#7c7d82]">{copy}</p></div></FadeIn>)}</div>
      </div>
    </section>

    <section id="audit-form" className="scroll-mt-24 border-b border-[#1a1c1e] bg-[#0d0f10] py-24">
      <div className="mx-auto grid w-[min(1112px,calc(100%-40px))] gap-12 lg:grid-cols-[0.82fr_1.18fr]">
        <FadeIn><div className="lg:sticky lg:top-28"><SectionHeading eyebrow="Request your audit" title="Describe the friction, not the solution." copy="You do not need to know what technology you need. Tell us what the team keeps doing manually, where work gets stuck, and which process feels harder to run than it should." /><div className="mt-8 space-y-3 text-sm text-[#77787e]"><p className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#d8cbb7]" />No commitment to a project</p><p className="flex items-center gap-2"><Users className="h-4 w-4 text-[#d8cbb7]" />Built for established businesses with real operations</p></div></div></FadeIn>
    ...
    <section className="flex min-h-[390px] items-center border-b border-[#1a1c1e] bg-[#0d0f10] text-center"><FadeIn className="mx-auto flex w-[min(1112px,calc(100%-40px))] flex-col items-center"><p className="text-[11px] font-extrabold uppercase tracking-[0.28em] text-[#d8cbb7]">Automation Audit</p><h2 className="mt-7 max-w-[830px] text-[clamp(38px,4.1vw,52px)] font-semibold leading-[1.06] tracking-[-0.045em]">See what is actually worth automating in your business.</h2><p className="mt-6 text-[17px] text-[#929399]">One conversation. No commitment. Real answers.</p><div className="mt-8"><PrimaryLink /></div></FadeIn></section>
...
function ResultCard({ icon: Icon, label, value, accent = false }: { icon: typeof Clock3; label: string; value: string; accent?: boolean }) {
  return <div className="rounded-xl border border-[#2b2d30] bg-[#0b0d0e] p-5"><Icon className={`h-4 w-4 ${accent ? 'text-[#d92f3c]' : 'text-[#626369]'}`} /><p className={`mt-4 text-xl font-semibold tracking-[-0.035em] ${accent ? 'text-[#efe3cf]' : 'text-[#f4f4f3]'}`}>{value}</p><p className="mt-1 text-xs leading-5 text-[#626369]">{label}</p></div>
}
